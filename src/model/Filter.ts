...
export class Filter implements IFilter {
    ...
    private data: Partial< F.TFilterCollection > = {};
    ...
    private prepFilter ( list: F.TFilterItem[] ) : F.TFilterItem[] {
        return [ ...new Map( list.map( i => [ i.uri, i ] ) ).values() ].sort(
            ( a, b ) => a.uri.localeCompare( b.uri )
        );
    }

    private setFilterData ( group: TFilterGroup, key: string, filter: F.TFilter ) : void {
        ( this.data[ group ] ??= {} as any )[ key ] = filter;
    }

    // Load & save filter

    private loadFilter ( group: TFilterGroup, key: string ) : F.TFilter | undefined {
        ...
        return log.catch( () => {
            ...
            const filter = Filter.storage.readJSON< F.TFilter >( resolved );
    ...
    private saveFilter ( group: TFilterGroup, key: string, list: F.TFilterItem[] ) : void {
        ...
        log.catch( () => {
            ...
            if ( ! Filter.storage.writeJSON< F.TFilter >( resolved, filter ) ) {
                throw new Error( `Failed to write filter file: ${resolved}` );
            }
        }, `Failed to save filter at ${group}/${key}` );
    }

    private saveGroup ( group: TFilterGroup, data: Record< string, F.TFilterItem[] > ) : void {
        Object.entries( data ).forEach( ( [ key, list ] ) => this.saveFilter( group, key, list ) );
    }

    private saveSpecial ( special: TFilterSpecial, data: F.TFilterItem[] ) : void {
        this.saveFilter( 'special', special, data );
    }

    // Get filter

    public getFilter ( group: TFilterGroup, key: string ) : F.TFilter | undefined {
        return ( this.data[ group ] as any )?.[ key ] ?? this.loadFilter( group, key );
    }

    public getFilterByPath ( path: string ) : F.TFilter | undefined {
        const [ group, key ] = this.splitPath( path ) ?? [];
        return group && key ? this.getFilter( group, key ) : undefined;
    }

    public getGroup ( group: TFilterGroup ) : Record< string, F.TFilter > | undefined {
    ...
    public getSpecial ( special: TFilterSpecial ) : F.TFilter | undefined {
    ...
    public save ( col: Partial< F.TFilterList > ) : void {
